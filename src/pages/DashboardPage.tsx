...
export default function DashboardPage() {
  ...
  const { assessment: ctxAssessment, recommendations: ctxRecs, formData, language, currentUser } = useApp();
  ...
  useEffect(() => {
    // If no context data (direct navigation), use demo data
    if (!assessment) {
      const demoAssessment = computeRiskAssessment(DEMO_FORM);
      const demoRecs = generateRecommendations(demoAssessment, DEMO_FORM);
  ...
  useEffect(() => {
    if (assessment) {
      const tsh = parseFloat(formData?.tsh || DEMO_FORM.tsh) || 5.8;
